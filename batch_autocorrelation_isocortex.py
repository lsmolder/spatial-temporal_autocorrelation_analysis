...
Two modes:
  ...
  COHORT MODE (default, lecanemab project):
    ...
    python batch_autocorrelation_isocortex.py \
...
from region_filter import get_isocortex_labels, filter_fc_and_ts
...
def process_scan(ts_path, fc_path, atlas_data, atlas_affine,
                 isocortex_labels, scan_out, sub, ses, out_run, cohort=None):
    fc_matrix, ts_data, kept_labels = filter_fc_and_ts(
        fc_path, ts_path, isocortex_labels
    )
    print(f"    Regions after isocortex filter: {len(kept_labels)}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch TA/SA — Isocortex only"
    )
    ...
    print("Loading isocortex label definitions from hierarchy...")
    isocortex_labels = get_isocortex_labels(args.hierarchy)
    print(f"  {len(isocortex_labels)} isocortical label values identified")
        ...
        for ts_path in ts_files:
            ...
            try:
                result = process_scan(
                    ...
                    isocortex_labels, scan_out, sub, ses, out_run
                )
        ...
        for cohort in COHORT_NAMES:
            ...
            for ts_path in ts_files:
                ...
                try:
                    result = process_scan(
                        ts_path, fc_path, atlas_data, atlas_affine,
                        isocortex_labels, scan_out, sub, ses, out_run, cohort
                    )
                    summary_results.append(result)
                except Exception as e:
                    print(f"    ❌ Error: {e}")
    ...
    if summary_results:
        ...
        out_path = os.path.join(
            args.output_dir, 'all_subjects_isocortex_autocorrelation.csv')
